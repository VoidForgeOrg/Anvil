// Borrowed from: https://github.com/dotnet/aspire-samples/blob/main/samples/Shared/DevCertHostingExtensions.cs
...
internal static class DevCertHostingExtensions
{
    ...
    public static IResourceBuilder<TResource> RunWithHttpsDevCertificate<TResource>(this IResourceBuilder<TResource> builder, string certFileEnv, string certKeyFileEnv,
        Action<string, string>? onSuccessfulExport = null) where TResource : IResourceWithEnvironment {
            ...
            builder.ApplicationBuilder.Eventing.Subscribe<BeforeStartEvent>(
                async (e, ct) =>
                {
                    ...
                    if (builder.Resource is ContainerResource containerResource) {
                        ...
                        builder.WithEnvironment(certFileEnv, certPath).WithEnvironment(certKeyFileEnv, certKeyPath);
                    }
    ...
    public static async Task<(bool, string CertFilePath, string CertKeyFilPath)> TryExportDevCertificateAsync(IDistributedApplicationBuilder builder, ILogger logger) {
        ...
        var tempDir = Path.Combine("./Properties/", $"aspire.{appNameHash}");
